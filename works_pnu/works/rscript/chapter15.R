...
product <- read.csv("C:/Rwork/Part-III/product.csv", 
                    header = T, 
                    fileEncoding = "euc-kr") # multibyte string -> 인코딩설정
str(product)
#머신러닝 데이터는 숫자로 바꿔야함. 
#ds 리코딩해서 설명넣어서 보는게 좋음.

## 단계 2: 독립변수와 종속변수 생성 -> 통계학 base
y = product$제품_만족도 
x = product$제품_적절성
df <- data.frame(x, y)
...
#매트릭스 
#테이블 - 헤더가 있음. 
#데이터 프레임 - 테이블에 비해서 메서드가 다양함.
#하나에 맞춰서 쓰기 파이썬을 r에 맞춰서 비슷하게 쓰기.

## 단계 3: 단순 선형회귀 "모델" 생성 -> 기울기와 절편을 생성하는 것.
...
# 모델의 잔차와 회귀방정식에 의한 적합값으로부터 관측값 계산 -> 큰지작은지? 비율로 계산
-0.7359630 + 3.735963
...
# x는 문제 y는 답안 선->모델(잔차가 가장 적음) -> 더 좋은게 있을 수 있나? 선형모델을 안쓰면 됨.

## 단계 4: 선형 회귀분석 결과보기 
summary(result.lm)
# p-velure x,y 설정 괜찮음 
# R-squared (->설명력) : 0.5881 백만원을 더 벌 었을 때 데이터로 58.81% 까지는 설명 가능.

#다중 회귀분석 -> x가 여러개 x1,x2 사이의 관계 검증필요 거의안씀.
...
setwd()
house <-read.csv("C:/works/data/housing.csv")
head(house)
...
par(mfrow=c(2,5))
colnames(house)
# ggplot(data= melt(house), mapping = aes(x=value))+geom_histogram(bins=30)+facet_wrap(-variables, scale='free_x')
...
house = house[ , !(names(house) %in% drop)] #정규화에 가깝지만 결측
head(house)             

str(house) #->chr 거슬려 num으로

## 전처리(상식을 사용해서 가정에 대한 데이터를 변도로 분리***)
...
??ggplot
# 1. 통계청 출산 데이터 (시계열, 회귀)
# 2. ~ 집 값 데이터(캘리포니아) => 대표적인 회귀, 시각화(위,경도 data 있음.) /보스턴집값데이터XXXXXX
# 3. 손글씨 분류 => (0~9분류)

#=========================로지스틱===============================

# 실습3: 날씨 관련 요인 변수로 비(rain) 유무 예측
## 단계 1: 데이터 가져오기 
